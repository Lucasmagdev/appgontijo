      ...
      {(!isEditing || obraQuery.data) && <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <ObraSection title="Questões Contratuais" sectionKey="contrato" activeSection={activeSection} setActiveSection={setActiveSection}>
          <div className="form-grid">
            ...
            <div style={gridSpan(4)}><label className="field-label">Valor Faturamento Mínimo (R$)</label><input className="field-input" type="number" min={0} step={0.01} placeholder="Ex: 1000.00" value={form.fatMinimoValor ?? ''} onChange={(event) => setField('fatMinimoValor', parseNumberInput(event.target.value))} /></div>
            ...
            <div style={gridSpan(4)}><label className="field-label">Acréscimo Bit (%)</label><input className="field-input" type="number" min={0} max={500} step={1} placeholder="Ex: 100" value={form.acrescimoBitPct ?? ''} onChange={(event) => setField('acrescimoBitPct', parseNumberInput(event.target.value))} /></div>
