...
import { productionService, type ProductionDailyDashboard, type ProductionWeeklyDashboard, type ProductionMachineRow } from '@/lib/gontijo-api'
...
function toneBg(percent: number | null) {
...
function CardEstacasDia({ data }: { data: ProductionDailyDashboard }) {
...
function CardMaquinaDestaque({ machine }: { machine: ProductionMachineRow | undefined }) {
  if (!machine) {
    return (
...
function CardObrasDestaque({ data }: { data: ProductionDailyDashboard }) {
...
function GridMaquinas({ machines }: { machines: ProductionMachineRow[] }) {
...
function Timeline({ data }: { data: ProductionDailyDashboard }) {
...
function PrimaryView({ data }: { data: ProductionDailyDashboard }) {
...
function TvTrendChart({ labels, primary, secondary }: {
  labels: string[]
  primary: number[]
  secondary?: number[]
}) {
...
function SecondaryView({ data }: { data: ProductionWeeklyDashboard }) {
...
const REFRESH_DAILY = 60
const REFRESH_WEEKLY = 300
