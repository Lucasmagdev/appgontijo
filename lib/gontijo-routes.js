...
function buildDiaryPdfBuffer(diario) {
  return new Promise((resolve, reject) => {
    ...
    const chunks = []
    const data = diario.dados_json && typeof diario.dados_json === 'object' ? diario.dados_json : {}
    ...
    doc.font('Helvetica-Bold').fontSize(18).fillColor('#111827').text('Diario de Obra')
    doc.moveDown(0.4)
    line('ID', String(diario.id))
    line('Obra', diario.obra_numero || '-')
    line('Cliente', diario.cliente || '-')
    line('Data', diario.data_diario ? new Date(diario.data_diario).toLocaleDateString('pt-BR') : '-')
    line('Status', diario.status || '-')
    line('Equipamento', diario.equipamento || '-')
    line('Operador', diario.operador_nome || '-')
    line('Assinado em', diario.assinado_em ? new Date(diario.assinado_em).toLocaleString('pt-BR') : '-')

    sectionTitle('Dados operacionais')
    line('Cliente na obra', data.client || '-')
    line('Inicio', data.start || '-')
    line('Fim', data.end || '-')
    line('Modalidade', data.modality || '-')
    line('Horimetro', data.horimetro || '-')
    line('Assinante', data.signatureName || '-')
    line('Documento', data.signatureDoc || '-')

    const addressParts = [data.address?.street, data.address?.number, data.address?.neighborhood].filter(Boolean)
    line('Endereco', addressParts.join(', ') || '-')

    sectionTitle('Equipe')
    const staff = Array.isArray(data.staff) ? data.staff : []
    if (staff.length) {
      staff.forEach((person, index) => {
        doc.font('Helvetica').fontSize(9).fillColor('#0f172a').text(`${index + 1}. ${person?.item || '-'}`)
      })
    } else {
      doc.font('Helvetica').fontSize(9).text('Sem equipe registrada.')
    }
    ...
    if (stakes.length) {
      stakes.forEach((stake, index) => {
        doc
          ...
          .fillColor('#0f172a')
          .text(
            `${index + 1}. ${stake?.stake || '-'} | Diametro: ${stake?.diameter || '-'} | Metros: ${stake?.meters || '-'} | Justificativa: ${stake?.justify || '-'}`
          )
      })
    } else {
      doc.font('Helvetica').fontSize(9).text('Sem estacas registradas.')
    }
    ...
    if (occurrences.length) {
      occurrences.forEach((occurrence, index) => {
        doc
          ...
          .fillColor('#0f172a')
          .text(
            `${index + 1}. ${occurrence?.desc || '-'} | Inicio: ${occurrence?.hora_ini || '-'} | Fim: ${occurrence?.hora_fim || '-'}`
          )
      })
    } else {
      doc.font('Helvetica').fontSize(9).text('Sem ocorrencias registradas.')
    }

    sectionTitle('JSON bruto')
    doc.font('Courier').fontSize(7).fillColor('#334155').text(JSON.stringify(data, null, 2))
    doc.end()
  })
}
...
router.get('/diarios/:id/pdf', async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `inline; filename="diario-${diario.id}.pdf"`)
