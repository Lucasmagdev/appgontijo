...
let usersJobTitleColumnEnsured = false
async function ensureUsersJobTitleColumn() {
  if (usersJobTitleColumnEnsured) return
  const exists = await legacyColumnExists('users', 'job_title')
  if (!exists) {
    await db.query("ALTER TABLE users ADD COLUMN job_title VARCHAR(120) NULL AFTER alias")
  }
  usersJobTitleColumnEnsured = true
}
