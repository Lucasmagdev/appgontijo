...
router.post('/usuarios', requireGontijoAdmin, async (req, res) => {
  try {
    const { nome, apelido, login, telefone, cargo, senha, perfil } = req.body
    if (!nome || !login || !senha) {
    ...
    const [result] = await db.query(
      'INSERT INTO users (name, alias, document, phone, cargo, password, active) VALUES (?,?,?,?,?,?,?)',
      [nome, textOrNull(apelido), login, textOrNull(telefone), textOrNull(cargo), senha_hash, perfil === 'admin' ? 'S' : 'S']
    )
...
router.put('/usuarios/:id', requireGontijoAdmin, async (req, res) => {
  try {
    const { nome, apelido, login, telefone, cargo, perfil, status } = req.body
    await db.query(
      'UPDATE users SET name = ?, alias = ?, document = ?, phone = ?, cargo = ?, active = ? WHERE id = ?',
      [nome, textOrNull(apelido), login, textOrNull(telefone), textOrNull(cargo), mapStatusToLegacyActive(status), req.params.id]
    )
    ok(res, {})
  } catch (e) {
    if (e.code === 'ER_DUP_ENTRY') return err(res, 'Login ja cadastrado')
