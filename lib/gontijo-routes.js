...
function buildMedicaoPdfBuffer(detalhe) {
  return new Promise((resolve, reject) => {
    ...
    // Logo area
    doc.save(); doc.fillColor(HEADER_BG).rect(L, y, W, 46).fill(); doc.restore()
    doc.save(); doc.strokeColor(BORDER).lineWidth(0.5).rect(L, y, W, 46).stroke(); doc.restore()

    if (fs.existsSync(DIARY_LOGO_PATH)) {
      try { doc.image(DIARY_LOGO_PATH, L + 4, y + 4, { fit: [80, 38], align: 'left', valign: 'center' }) } catch {}
    }

    const titleX = L + 90
    const titleW = W - 90 - 120
    doc.font('Helvetica-Bold').fontSize(11).fillColor('#111')
       .text(`Relatório Estacas - ${medicao.obra_numero || ''}`, titleX, y + 8, { width: titleW, align: 'center', lineBreak: false })
    doc.font('Helvetica').fontSize(7.5).fillColor('#444')
       .text(`${medicao.cliente || ''}`, titleX, y + 24, { width: titleW, align: 'center', lineBreak: false })
    ...
    for (const [diam, preco] of sortedDias.slice(0, 3)) {
      ...
      doc.text(String(diam), px + 2, priceRowY, { width: pW[0] - 4, align: 'center', lineBreak: false })
    ...
    // Info rows
    const infoRows = [
      [['CLIENTE:', medicao.cliente], ['EQUIPAMENTO:', ''], ['DATA:', formatDateBr(medicao.data_medicao)]],
      [['ENDEREÇO:', [medicao.endereco, medicao.endereco_num, medicao.bairro].filter(Boolean).join(', ')], [], ['OPERADOR:', '']],
      [['', ''], ['N° DO CONTRATO:', medicao.obra_numero], ['N° DA MEDIÇÃO:', String(medicao.numero || '')], ['N° DA REVISÃO:', String(medicao.revisao ?? '0')]],
    ]

    const infoRowH = 16
    for (const rowDef of infoRows) {
      doc.save(); doc.fillColor('#fafafa').rect(L, y, W, infoRowH).fill(); doc.restore()
      doc.save(); doc.strokeColor(BORDER).lineWidth(0.5).rect(L, y, W, infoRowH).stroke(); doc.restore()
      const segW = W / Math.max(1, rowDef.filter(r => r && r[0]).length)
      let sx = L
      for (const seg of rowDef) {
        if (!seg || !seg[0]) continue
        doc.font('Helvetica-Bold').fontSize(6).fillColor('#111')
           .text(seg[0], sx + 3, y + 5, { width: 70, lineBreak: false })
        doc.font('Helvetica').fontSize(6.5)
           .text(String(seg[1] || ''), sx + 75, y + 5, { width: segW - 80, lineBreak: false })
        sx += segW
      }
      y += infoRowH
    }
    ...
    doc.save(); doc.strokeColor(BORDER).lineWidth(0.5).rect(L, y, W, infoRowH).stroke(); doc.restore()
    const halfW = W / 2
    doc.font('Helvetica-Bold').fontSize(6).fillColor('#111').text('Responsável Comercial:', L + 3, y + 5, { width: 90, lineBreak: false })
    doc.font('Helvetica').fontSize(6.5).text(String(medicao.responsavel_comercial || ''), L + 96, y + 5, { width: halfW - 100, lineBreak: false })
    vline(L + halfW, y, y + infoRowH)
    doc.font('Helvetica-Bold').fontSize(6).text('Responsável pela medição:', L + halfW + 3, y + 5, { width: 100, lineBreak: false })
    doc.font('Helvetica').fontSize(6.5).text(String(medicao.responsavel_medicao || ''), L + halfW + 106, y + 5, { width: halfW - 110, lineBreak: false })
    y += infoRowH + 4
    ...
    for (let idx = 0; idx < estacas.length; idx++) {
      ...
      const rowData = [
        ...
        e.diametro != null ? String(e.diametro) : '-',
        m > 0 ? fmtBRL(m) : '-',
