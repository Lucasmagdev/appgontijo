...
router.patch('/obras/:id/fotos', async (req, res) => {
  try {
...
router.get('/obras/:numero/tolerancia-conferencia', async (req, res) => {
  try {
    await ensureToleranciaColumn()
    const [[row]] = await db.query(
...
router.post('/conferencia-estacas/:id/estacas/:stakeIndex/status', async (req, res) => {
  const conn = await db.getConnection()
