...
function buildDiaryPdfBuffer(diario) {
  return new Promise((resolve, reject) => {
    ...
    const row1Y = top + 44
    drawBox(doc, left, row1Y, headerLeftW, 30, { stroke: border })
    drawBox(doc, headerCenterX, row1Y, headerCenterW, 30, { stroke: border })
    drawBox(doc, headerRightX, row1Y, headerRightW, 30, { stroke: border })
    ...
    drawBox(doc, left, row2Y, headerLeftW, 30, { stroke: border })
    drawBox(doc, headerCenterX, row2Y, headerCenterW, 30, { stroke: border })
    drawBox(doc, headerRightX, row2Y, headerRightW, 30, { stroke: border })
    ...
    drawBox(doc, left, row3Y, headerLeftW, 30, { stroke: border })
    drawBox(doc, headerCenterX, row3Y, headerCenterW, 30, { stroke: border })
    drawBox(doc, headerRightX, row3Y, headerRightW, 30, { stroke: border })
    ...
    let weatherY = row3Y + 38
    if (ctx.isDrivenPile) {
      const infoY = row3Y + 38
...
router.post('/cursos/:id/atribuicoes', async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      ...
      [req.params.id, tipo, intOrNull(req.body.setor_id), intOrNull(req.body.usuario_id), tipoAcesso]
    )
    const [[row]] = await db.query(
  ...
  try {
    ...
    const [cursos] = await db.query(
      `SELECT c.*,
       ...
       WHERE c.ativo = 1
         AND (a.usuario_id = ? OR (a.tipo = 'setor' AND a.setor_id = ?))
       GROUP BY c.id
...
router.get('/operador/cursos/:id', async (req, res) => {
  try {
    ...
    ok(res, { data: { ...curso, prova: prova ?? null, ultima_tentativa: ultimaTentativa, tipo_acesso: tipoAcessoRow?.tipo_acesso || 'curso_e_prova' } })
  } catch (e) { err(res, e.message, 500) }
})
