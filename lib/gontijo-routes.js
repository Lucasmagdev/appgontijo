  ...
  try {
    ...
    const resolvedSignedAt =
      effectiveStatus === 'assinado'
        ? textOrNull(assinado_em ?? currentData.assinado_em) || new Date().toISOString().slice(0, 19).replace('T', ' ')
        : null
    const duplicateDiary = await findDiaryByEquipmentAndDate(conn, {
      equipmentId: equipamento_id,
  ...
  try {
    ...
    if (obraIds.length) {
      ...
      try {
        ...
        const [tolRows] = await db.query(
          'SELECT id, tolerancia_conferencia FROM constructions WHERE id IN (?)',
          [obraIds]
        )
        for (const t of tolRows) {
          if (t.tolerancia_conferencia != null) {
            toleranciaMap.set(Number(t.id), Number(t.tolerancia_conferencia) / 100)
          }
        }
      } catch { /* usa padrão 10% */ }
    }
    ...
    const items = rows.map((row) => {
      ...
      return {
        ...
        equipamentoId: row.equipamento_id ? Number(row.equipamento_id) : null,
...
const PERDAS_VALIDAS = new Set([
  'Aguardando início da obra','Aguardando liberação de projeto do cliente','Aguardando projeto interno',
  'Dificuldade de perfuração devido ao terreno','Estacas curtas / diâmetro menor','Falta de área de trabalho',
  'Falta de liberação da obra','Interferências na obra','Mudança de projeto','Obra embargada',
  'Paralisação do cliente','Problema com vizinhança','Reunião com cliente','Revisão de projeto',
  'Aguardando documentação de equipe',
  'Aguardando documentação (equipamento)','Aguardando manutenção','Manutenção da máquina em obra',
  'Manutenção da máquina no pátio',"Problemas com a bomba d'água",'Problemas hidráulicos',
  'Problemas elétricos','Quebra de equipamento','Troca de peças','Vazamentos',
  'Reparo de trados e/ou ponteiras','Adaptações e melhorias','Manutenção preventiva / melhorias','Pintura',
  '(Des)Montagem do equipamento','Aguardando armação / trilho','Aguardando bomba de concreto',
  'Aguardando concretagem das estacas','Aguardando concreto','Aguardando ferragem','Aguardando locação',
  'Aguardando topografia','Falta de insumos','Limpeza da área','Montagem de armação',
  'Reposicionamento de equipamento','Testes operacionais',
  'Aguardando peças','Aguardando transporte','Mobilização da máquina','Desmobilização da máquina',
  'Problemas com transporte',
  'Chuva','Domingo','Feriado','Sábado ocioso','Terreno inacessível','Ventos fortes',
  'Aguardando execução de ensaios','Execução de ensaios',
  'Diário não enviado','Erro operacional',
...
router.post('/conferencia-estacas/:id/aprovar', requireGontijoAdmin, async (req, res) => {
  try {
    ...
    if (!metaBool) {
      if (!perda) return err(res, 'Selecione uma perda')
      if (!PERDAS_VALIDAS.has(perda)) return err(res, 'Perda inválida')
    }

    const [result] = await db.query(
      `UPDATE diaries
       SET conferencia_status = 'aprovado', conferencia_em = NOW(), conferencia_por = ?,
           conferencia_obs = ?, meta_atingida = ?, perda = ?
       WHERE id = ?`,
      [adminId, obs, metaBool ? 1 : 0, perda, id]
    )
...
// ── Planejamento Diário de Máquinas ───────────────────────────────────────────
...
router.get('/planejamento-diario', requireGontijoAdmin, async (req, res) => {
  try {
    ...
    let where = '1=1'
    const params = []
    if (dataInicio) { where += ' AND p.data >= ?'; params.push(dataInicio) }
    if (dataFim)    { where += ' AND p.data <= ?'; params.push(dataFim) }
    if (equipamentoId) { where += ' AND p.equipamento_id = ?'; params.push(equipamentoId) }

    const [rows] = await db.query(
      `SELECT p.id, p.data, p.equipamento_id, p.obra_id, p.fat_minimo_garantido,
              e.nome AS equipamento_nome,
              o.construction_number AS obra_numero
       FROM planejamento_diario p
       LEFT JOIN equipamentos e ON e.id = p.equipamento_id
       LEFT JOIN constructions o ON o.id = p.obra_id
       WHERE ${where}
       ORDER BY p.data DESC, e.nome`,
      params
    )
    ...
    if (ids.length > 0) {
      const [itensRows] = await db.query(
        `SELECT * FROM planejamento_diario_itens WHERE planejamento_id IN (${ids.map(() => '?').join(',')})`,
        ids
      )
      itens = itensRows
    }
    ...
    const data = rows.map(r => ({
      ...
      obraId: r.obra_id,
      obraNumero: r.obra_numero || '',
      cliente: '',
      fatMinimoGarantido: !!r.fat_minimo_garantido,
      itens: itens.filter(i => i.planejamento_id === r.id).map(i => ({
        id: i.id,
        metaQtdEstacas: i.meta_qtd_estacas,
        diametro: i.diametro,
        profundidade: Number(i.profundidade),
      })),
    }))
...
router.post('/planejamento-diario', requireGontijoAdmin, async (req, res) => {
  try {
    const adminId = intOrNull(req.session?.adminUserId)
    const { data_inicio, data_fim, equipamento_id, obra_numero, fat_minimo_garantido, itens } = req.body

    if (!data_inicio) return err(res, 'data_inicio obrigatória')
    const equipId = intOrNull(equipamento_id)
    if (!equipId) return err(res, 'equipamento_id inválido')
    const obraNumeroStr = textOrNull(obra_numero)
    if (!obraNumeroStr) return err(res, 'Informe o número da obra')
    const [[obraRow]] = await db.query('SELECT id FROM constructions WHERE construction_number = ?', [obraNumeroStr])
    if (!obraRow) return err(res, `Obra "${obraNumeroStr}" não encontrada`)
    const obraId = obraRow.id

    if (!Array.isArray(itens) || itens.length === 0) return err(res, 'Informe ao menos um item (diâmetro/quantidade)')

    const dataFim = textOrNull(data_fim) || data_inicio
    const dates = []
    let cur = new Date(data_inicio + 'T00:00:00')
    ...
    for (const date of dates) {
      ...
      if (existing.length > 0) {
        ...
        await db.query(
          'UPDATE planejamento_diario SET fat_minimo_garantido=?, criado_por=? WHERE id=?',
          [fat_minimo_garantido ? 1 : 0, adminId, planId]
        )
        await db.query('DELETE FROM planejamento_diario_itens WHERE planejamento_id=?', [planId])
      } else {
        const [r] = await db.query(
          'INSERT INTO planejamento_diario (data, equipamento_id, obra_id, fat_minimo_garantido, criado_por) VALUES (?,?,?,?,?)',
          [date, equipId, obraId, fat_minimo_garantido ? 1 : 0, adminId]
        )
        planId = r.insertId
      }
      for (const item of itens) {
        await db.query(
          'INSERT INTO planejamento_diario_itens (planejamento_id, meta_qtd_estacas, diametro, profundidade) VALUES (?,?,?,?)',
          [planId, intOrNull(item.meta_qtd_estacas) || 0, String(item.diametro || ''), Number(item.profundidade) || 0]
        )
      }
      created.push(planId)
    }
...
router.put('/planejamento-diario/:id', requireGontijoAdmin, async (req, res) => {
  try {
    const id = intOrNull(req.params.id)
    if (!id) return err(res, 'ID inválido')
    const { fat_minimo_garantido, itens } = req.body

    const [[plan]] = await db.query('SELECT id FROM planejamento_diario WHERE id=?', [id])
    if (!plan) return err(res, 'Não encontrado', 404)

    await db.query('UPDATE planejamento_diario SET fat_minimo_garantido=? WHERE id=?', [fat_minimo_garantido ? 1 : 0, id])

    if (Array.isArray(itens)) {
      await db.query('DELETE FROM planejamento_diario_itens WHERE planejamento_id=?', [id])
      for (const item of itens) {
        await db.query(
          'INSERT INTO planejamento_diario_itens (planejamento_id, meta_qtd_estacas, diametro, profundidade) VALUES (?,?,?,?)',
          [id, intOrNull(item.meta_qtd_estacas) || 0, String(item.diametro || ''), Number(item.profundidade) || 0]
        )
      }
    }
...
router.delete('/planejamento-diario/:id', requireGontijoAdmin, async (req, res) => {
  try {
    const id = intOrNull(req.params.id)
    if (!id) return err(res, 'ID inválido')
    await db.query('DELETE FROM planejamento_diario WHERE id=?', [id])
    ok(res, {})
  } catch (e) { err(res, e.message, 500) }
})
...
function calcMedicaoTotais(medicao, estacas, extras) {
  const valorEstacas = round2(estacas.reduce((s, e) => s + Number(e.custo_total || 0), 0))
  const valorExtras  = round2(extras.reduce((s, e)  => s + Number(e.valor      || 0), 0))
  const valorTotal   = round2(valorEstacas + valorExtras)

  const pctNf               = Number(medicao.pct_nf      || 0)
  const pctLocacao          = Number(medicao.pct_locacao  || 0)
  const issqnPct            = Number(medicao.issqn_pct   || 0)
  const issqnCobradoCliente = Boolean(medicao.issqn_cobrado_cliente)

  const valorNf      = pctNf     > 0 ? round2(valorTotal * pctNf     / 100) : valorTotal
  const valorLocacao = pctLocacao > 0 ? round2(valorTotal * pctLocacao / 100) : 0
  const valorIssqn   = issqnPct  > 0 ? round2(valorNf    * issqnPct  / 100) : 0
  const valorLiquido = round2(issqnCobradoCliente ? valorNf + valorIssqn : valorNf - valorIssqn)

  return { valorEstacas, valorFatMinimo: 0, valorExtras, valorTotal,
    issqnPct, pctNf, pctLocacao, issqnCobradoCliente,
    valorNf, valorLocacao, valorIssqn, valorLiquido }
}

async function getMedicaoDetalhe(id) {
  const [[medicao]] = await db.query(
    `SELECT m.*,
            c.construction_number AS obra_numero,
            cl.name AS cliente,
            CONCAT_WS(', ', NULLIF(c.city,''), NULLIF(c.state,'')) AS endereco,
            c.responsavel_comercial_gontijo AS responsavel_comercial
     FROM medicoes m
  ...
  const [estacas] = await db.query(
    'SELECT * FROM medicoes_estacas      WHERE medicao_id=? ORDER BY ordem, id', [id])
  const [extras]  = await db.query(
    'SELECT * FROM medicoes_itens_extras WHERE medicao_id=? ORDER BY ordem, id', [id])

  return {
    medicao: {
      id: Number(medicao.id),
      construction_id: Number(medicao.construction_id),
      numero: Number(medicao.numero),
      data_medicao: medicao.data_medicao ? String(medicao.data_medicao).slice(0,10) : null,
      data_inicio:  medicao.data_inicio  ? String(medicao.data_inicio).slice(0,10)  : null,
      data_fim:     medicao.data_fim     ? String(medicao.data_fim).slice(0,10)     : null,
      responsavel_medicao:   medicao.responsavel_medicao   || null,
      conferido_por:         medicao.conferido_por         || null,
      status:                medicao.status                || 'rascunho',
      fat_minimo_valor:      medicao.fat_minimo_valor      != null ? Number(medicao.fat_minimo_valor) : null,
      issqn_pct:             medicao.issqn_pct             != null ? Number(medicao.issqn_pct)        : null,
      pct_nf:                medicao.pct_nf                != null ? Number(medicao.pct_nf)           : null,
      pct_locacao:           medicao.pct_locacao           != null ? Number(medicao.pct_locacao)      : null,
      issqn_cobrado_cliente: Number(medicao.issqn_cobrado_cliente || 0),
      obra_numero:           medicao.obra_numero           || '',
      cliente:               medicao.cliente               || '',
      endereco:              medicao.endereco              || '',
      responsavel_comercial: medicao.responsavel_comercial || null,
    },
    estacas: estacas.map(e => ({
      ...e,
      id: Number(e.id), medicao_id: Number(e.medicao_id),
      diametro:    e.diametro    != null ? Number(e.diametro)    : null,
      profundidade:e.profundidade!= null ? Number(e.profundidade): null,
      valor_metro: e.valor_metro != null ? Number(e.valor_metro) : null,
      uso_bits:    Number(e.uso_bits || 0),
      metros_armacao:     e.metros_armacao      != null ? Number(e.metros_armacao)      : null,
      valor_armacao_metro:e.valor_armacao_metro != null ? Number(e.valor_armacao_metro) : null,
      custo_total: e.custo_total != null ? Number(e.custo_total) : null,
      ordem:       Number(e.ordem || 0),
      data_estaca: e.data_estaca ? String(e.data_estaca).slice(0,10) : null,
    })),
    extras: extras.map(e => ({ ...e, id: Number(e.id), medicao_id: Number(e.medicao_id), valor: Number(e.valor) })),
    fatMinimoTable: [],
    totais: calcMedicaoTotais(medicao, estacas, extras),
  }
}

// GET /gontijo/medicoes
router.get('/medicoes', requireGontijoAdmin, async (req, res) => {
  try {
    const { page, limit, offset } = paginate(req)
    const obraId = intOrNull(req.query.obraId)
    let where = 'WHERE 1=1'; const params = []
    if (obraId) { where += ' AND m.construction_id=?'; params.push(obraId) }
    ...
    const [rows] = await db.query(
      `SELECT m.id, m.construction_id, m.numero, m.data_medicao, m.data_inicio, m.data_fim,
              m.responsavel_medicao, m.conferido_por, m.status,
              c.construction_number AS obra_numero, cl.name AS cliente
       FROM medicoes m
       LEFT JOIN constructions c  ON c.id  = m.construction_id
       LEFT JOIN clients       cl ON cl.id = c.client_id
    ...
    ok(res, { total, page, limit,
      items: rows.map(r => ({
        id: Number(r.id), construction_id: Number(r.construction_id),
        numero: Number(r.numero),
        data_medicao: r.data_medicao ? String(r.data_medicao).slice(0,10) : null,
        data_inicio:  r.data_inicio  ? String(r.data_inicio).slice(0,10)  : null,
        data_fim:     r.data_fim     ? String(r.data_fim).slice(0,10)     : null,
        responsavel_medicao: r.responsavel_medicao || null,
        conferido_por: r.conferido_por || null,
        status: r.status || 'rascunho',
        obra_numero: r.obra_numero || '', cliente: r.cliente || '',
      })) })
  } catch (e) { err(res, e.message, 500) }
})

// POST /gontijo/medicoes
router.post('/medicoes', requireGontijoAdmin, async (req, res) => {
  try {
    const { obraId, dataInicio, dataFim, responsavelMedicao, conferidoPor, itensExtras } = req.body
    if (!obraId || !dataInicio || !dataFim) return err(res, 'obraId, dataInicio e dataFim são obrigatórios', 400)

    const [[{ next }]] = await db.query(
      'SELECT COALESCE(MAX(numero),0)+1 AS next FROM medicoes WHERE construction_id=?', [obraId])

    const [result] = await db.query(
      `INSERT INTO medicoes (construction_id, numero, data_inicio, data_fim, responsavel_medicao, conferido_por)
       VALUES (?,?,?,?,?,?)`,
      [obraId, next, dataInicio, dataFim, responsavelMedicao || null, conferidoPor || null])

    const medicaoId = result.insertId
    if (Array.isArray(itensExtras) && itensExtras.length) {
      for (let i = 0; i < itensExtras.length; i++) {
        await db.query(
          'INSERT INTO medicoes_itens_extras (medicao_id, descricao, valor, ordem) VALUES (?,?,?,?)',
          [medicaoId, itensExtras[i].descricao, Number(itensExtras[i].valor) || 0, i])
      }
    }
    ok(res, { id: medicaoId, numero: next })
  } catch (e) { err(res, e.message, 500) }
})

// GET /gontijo/medicoes/:id
router.get('/medicoes/:id', requireGontijoAdmin, async (req, res) => {
  try {
    const id = intOrNull(req.params.id)
    if (!id) return err(res, 'ID inválido', 400)
    const detalhe = await getMedicaoDetalhe(id)
    if (!detalhe) return err(res, 'Medição não encontrada', 404)
    ok(res, { data: detalhe })
  } catch (e) { err(res, e.message, 500) }
})

// PUT /gontijo/medicoes/:id
router.put('/medicoes/:id', requireGontijoAdmin, async (req, res) => {
  try {
    const id = intOrNull(req.params.id)
    if (!id) return err(res, 'ID inválido', 400)
    const { responsavelMedicao, conferidoPor, dataMedicao, dataInicio, dataFim,
            itensExtras, issqnPct, pctNf, pctLocacao, issqnCobradoCliente } = req.body

    const sets = []; const vals = []
    if (responsavelMedicao !== undefined) { sets.push('responsavel_medicao=?'); vals.push(responsavelMedicao || null) }
    if (conferidoPor       !== undefined) { sets.push('conferido_por=?');       vals.push(conferidoPor       || null) }
    if (dataMedicao        !== undefined) { sets.push('data_medicao=?');        vals.push(dataMedicao        || null) }
    if (dataInicio         !== undefined) { sets.push('data_inicio=?');         vals.push(dataInicio         || null) }
    if (dataFim            !== undefined) { sets.push('data_fim=?');            vals.push(dataFim            || null) }
    if (issqnPct           !== undefined) { sets.push('issqn_pct=?');           vals.push(issqnPct           != null ? Number(issqnPct)    : null) }
    if (pctNf              !== undefined) { sets.push('pct_nf=?');              vals.push(pctNf              != null ? Number(pctNf)       : null) }
    if (pctLocacao         !== undefined) { sets.push('pct_locacao=?');         vals.push(pctLocacao         != null ? Number(pctLocacao)  : null) }
    if (issqnCobradoCliente!== undefined) { sets.push('issqn_cobrado_cliente=?');vals.push(issqnCobradoCliente ? 1 : 0) }

    if (sets.length) {
      vals.push(id)
      await db.query(`UPDATE medicoes SET ${sets.join(',')} WHERE id=?`, vals)
    }

    if (Array.isArray(itensExtras)) {
      await db.query('DELETE FROM medicoes_itens_extras WHERE medicao_id=?', [id])
      for (let i = 0; i < itensExtras.length; i++) {
        await db.query(
          'INSERT INTO medicoes_itens_extras (medicao_id, descricao, valor, ordem) VALUES (?,?,?,?)',
          [id, itensExtras[i].descricao, Number(itensExtras[i].valor) || 0, i])
      }
    }
    ok(res, {})
  } catch (e) { err(res, e.message, 500) }
...
// DELETE /gontijo/medicoes/:id
router.delete('/medicoes/:id', requireGontijoAdmin, async (req, res) => {
  try {
    const id = intOrNull(req.params.id)
    if (!id) return err(res, 'ID inválido', 400)
...
// PUT /gontijo/medicoes/:id/status
router.put('/medicoes/:id/status', requireGontijoAdmin, async (req, res) => {
  try {
    const id = intOrNull(req.params.id)
    if (!id) return err(res, 'ID inválido', 400)
    const status = req.body.status
    if (!['rascunho','fechada'].includes(status)) return err(res, 'Status inválido', 400)
    await db.query('UPDATE medicoes SET status=? WHERE id=?', [status, id])
    ok(res, {})
  } catch (e) { err(res, e.message, 500) }
})

// POST /gontijo/medicoes/:id/estacas
router.post('/medicoes/:id/estacas', requireGontijoAdmin, async (req, res) => {
  try {
    const medicaoId = intOrNull(req.params.id)
    if (!medicaoId) return err(res, 'ID inválido', 400)
    const { data_estaca, nome_estaca, diametro, profundidade, valor_metro,
            uso_bits, metros_armacao, valor_armacao_metro, custo_total, observacao, ordem } = req.body
    const [r] = await db.query(
      `INSERT INTO medicoes_estacas
         (medicao_id, data_estaca, nome_estaca, diametro, profundidade, valor_metro,
          uso_bits, metros_armacao, valor_armacao_metro, custo_total, observacao, ordem)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
      [medicaoId, data_estaca||null, nome_estaca||null,
       diametro!=null?Number(diametro):null, profundidade!=null?Number(profundidade):null,
       valor_metro!=null?Number(valor_metro):null,
       uso_bits?1:0,
       metros_armacao!=null?Number(metros_armacao):null,
       valor_armacao_metro!=null?Number(valor_armacao_metro):null,
       custo_total!=null?Number(custo_total):null,
       observacao||null, Number(ordem)||0])
    ok(res, { id: r.insertId })
  } catch (e) { err(res, e.message, 500) }
})

// PATCH /gontijo/medicoes/:id/estacas/:estacaId
router.patch('/medicoes/:id/estacas/:estacaId', requireGontijoAdmin, async (req, res) => {
  try {
    const medicaoId = intOrNull(req.params.id)
    const estacaId  = intOrNull(req.params.estacaId)
    if (!medicaoId || !estacaId) return err(res, 'ID inválido', 400)

    const allowed = ['data_estaca','nome_estaca','diametro','profundidade','valor_metro',
                     'uso_bits','metros_armacao','valor_armacao_metro','custo_total','observacao','ordem']
    const sets = []; const vals = []
    for (const key of allowed) {
      if (key in req.body) {
        ...
        const numKeys = ['diametro','profundidade','valor_metro','metros_armacao','valor_armacao_metro','custo_total','ordem']
        if (key === 'uso_bits') vals.push(v ? 1 : 0)
        else if (numKeys.includes(key)) vals.push(v != null && v !== '' ? Number(v) : null)
        else vals.push(v != null && v !== '' ? v : null)
      }
    }
    if (!sets.length) return ok(res, {})
    vals.push(estacaId, medicaoId)
    await db.query(`UPDATE medicoes_estacas SET ${sets.join(',')} WHERE id=? AND medicao_id=?`, vals)
    ok(res, {})
  } catch (e) { err(res, e.message, 500) }
})

// DELETE /gontijo/medicoes/:id/estacas/:estacaId
router.delete('/medicoes/:id/estacas/:estacaId', requireGontijoAdmin, async (req, res) => {
  try {
    const medicaoId = intOrNull(req.params.id)
    const estacaId  = intOrNull(req.params.estacaId)
    if (!medicaoId || !estacaId) return err(res, 'ID inválido', 400)
    await db.query('DELETE FROM medicoes_estacas WHERE id=? AND medicao_id=?', [estacaId, medicaoId])
    ok(res, {})
  } catch (e) { err(res, e.message, 500) }
})

// POST /gontijo/medicoes/:id/reimportar
router.post('/medicoes/:id/reimportar', requireGontijoAdmin, async (req, res) => {
  try {
    const id = intOrNull(req.params.id)
    if (!id) return err(res, 'ID inválido', 400)

    const [[medicao]] = await db.query(
      'SELECT construction_id, data_inicio, data_fim FROM medicoes WHERE id=?', [id])
    if (!medicao) return err(res, 'Medição não encontrada', 404)

    const diaryDateExpr = "COALESCE(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(d.data, '$.date')), ''), DATE_FORMAT(d.created_at, '%Y-%m-%d'))"
    const diaryConstrExpr = "CAST(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(d.data, '$.construction_id')), '') AS UNSIGNED)"

    const [diaries] = await db.query(
      `SELECT d.id, d.data AS raw_data, ${diaryDateExpr} AS data_diario
       FROM diaries d
       WHERE d.conferencia_status = 'aprovado'
         AND ${diaryConstrExpr} = ?
         AND ${diaryDateExpr} BETWEEN ? AND ?
       ORDER BY ${diaryDateExpr} ASC, d.id ASC`,
      [medicao.construction_id, medicao.data_inicio, medicao.data_fim])

    // Remove previously imported estacas (keep manually added ones)
    await db.query('DELETE FROM medicoes_estacas WHERE medicao_id=? AND origem_diario_id IS NOT NULL', [id])

    let totalImportadas = 0
    for (const diary of diaries) {
      const data = safeParseJson(diary.raw_data)
      const parsed = typeof data === 'object' && data ? data : {}
      const estacas = extractEstacasFromDiaryData(parsed)
      for (let i = 0; i < estacas.length; i++) {
        ...
        await db.query(
          `INSERT INTO medicoes_estacas
             (medicao_id, data_estaca, nome_estaca, diametro, profundidade, origem_diario_id, ordem)
           VALUES (?,?,?,?,?,?,?)`,
          [id, diary.data_diario || null, e.nome || null,
           e.diametro != null ? Number(e.diametro) : null,
           e.profundidade != null ? Number(e.profundidade) : null,
           Number(diary.id), totalImportadas + i])
        totalImportadas++
      }
    }
    ok(res, { estacasImportadas: totalImportadas })
  } catch (e) { err(res, e.message, 500) }
})

// PATCH /gontijo/medicoes/:id/dias/:data/observacao
router.patch('/medicoes/:id/dias/:data/observacao', requireGontijoAdmin, async (req, res) => {
  try {
    const medicaoId = intOrNull(req.params.id)
    const data = req.params.data
    if (!medicaoId || !data) return err(res, 'Parâmetros inválidos', 400)
    const obs = req.body.observacao != null ? String(req.body.observacao) : null
    await db.query(
      `INSERT INTO medicoes_obs_dia (medicao_id, data, observacao) VALUES (?,?,?)
...
// GET /gontijo/medicoes/:id/pdf
router.get('/medicoes/:id/pdf', requireGontijoAdmin, async (req, res) => {
  try {
    const id = intOrNull(req.params.id)
    if (!id) return err(res, 'ID inválido', 400)
    const detalhe = await getMedicaoDetalhe(id)
    ...
    const { medicao, estacas, extras, totais } = detalhe
    const doc = new PDFDocument({ size: 'A4', margin: 36 })
    const buffers = []
    doc.on('data', b => buffers.push(b))

    const fmtBRL = v => (v == null ? '-' : Number(v).toLocaleString('pt-BR', { minimumFractionDigits: 2 }))
    const W = 595 - 72 // usable width
    const L = 36       // left margin

    // Title
    doc.fontSize(13).font('Helvetica-Bold').text(`MEDIÇÃO Nº ${medicao.numero}`, L, 36, { align: 'center', width: W })
    doc.fontSize(9).font('Helvetica')
      .text(`Obra: ${medicao.obra_numero}  |  Cliente: ${medicao.cliente}`, L, 54, { align: 'center', width: W })
      .text(`Período: ${medicao.data_inicio || ''} a ${medicao.data_fim || ''}  |  Status: ${medicao.status}`, L, 66, { align: 'center', width: W })

    let y = 90

    // Services table header
    const col1W = W * 0.72; const col2W = W * 0.28
    doc.rect(L, y, W, 16).fillAndStroke('#c8d6e5', '#999')
    doc.fillColor('#000').fontSize(8).font('Helvetica-Bold')
      .text('Descrição Serviços', L + 4, y + 4, { width: col1W - 4 })
      .text('Valor Total (R$)', L + col1W + 4, y + 4, { width: col2W - 4, align: 'right' })
    y += 16

    // Stakes row
    const drawRow = (label, value, bg) => {
      doc.rect(L, y, W, 14).fillAndStroke(bg || '#fff', '#ddd')
      doc.fillColor('#000').fontSize(8).font('Helvetica')
        .text(label, L + 4, y + 3, { width: col1W - 4 })
        .text(value, L + col1W + 4, y + 3, { width: col2W - 8, align: 'right' })
      y += 14
    }

    if (estacas.length) drawRow('Medição das estacas', `R$ ${fmtBRL(totais.valorEstacas)}`, '#f9f9f9')
    for (const ex of extras) drawRow(ex.descricao, `R$ ${fmtBRL(ex.valor)}`, '#f9f9f9')

    // Total row
    doc.rect(L, y, W, 16).fillAndStroke('#eef2f7', '#aaa')
    doc.fillColor('#000').fontSize(8).font('Helvetica-Bold')
      .text('TOTAL :', L + 4, y + 4, { width: col1W - 4, align: 'right' })
      .text(`R$  ${fmtBRL(totais.valorTotal)}`, L + col1W + 4, y + 4, { width: col2W - 8, align: 'right' })
    y += 20

    // NF / Locação blocks
    const drawBlock = (label, value, bold) => {
      y += 6
      doc.rect(L, y, W, 18).fillAndStroke('#eef2f7', '#aaa')
      doc.fillColor('#000').fontSize(9).font(bold ? 'Helvetica-Bold' : 'Helvetica')
        .text(label, L + 6, y + 5, { width: W * 0.7 })
        .text(`R$  ${fmtBRL(value)}`, L + W * 0.7, y + 5, { width: W * 0.3 - 6, align: 'right' })
      y += 18
    }

    const pctNfLabel = totais.pctNf > 0 ? ` (${totais.pctNf}%)` : ''
    const pctLocLabel = totais.pctLocacao > 0 ? ` (${totais.pctLocacao}%)` : ''
    const brutoNf = totais.issqnCobradoCliente ? totais.valorNf + totais.valorIssqn : totais.valorNf
    const liquidoNf = totais.issqnCobradoCliente ? totais.valorNf : totais.valorNf - totais.valorIssqn

    drawBlock(`VALOR BRUTO (R$) - Nota Fiscal${pctNfLabel}`, brutoNf, true)

    if (totais.issqnPct > 0) {
      y += 4
      doc.rect(L, y, W, 14).fillAndStroke('#fff', '#ddd')
      doc.fillColor('#000').fontSize(8).font('Helvetica')
        .text('ISSQN', L + 4, y + 3)
        .text(`Alíquota: ${totais.issqnPct}%`, L + W * 0.5, y + 3, { width: W * 0.3, align: 'right' })
        .text(`R$ ${fmtBRL(totais.valorIssqn)}`, L + W * 0.8, y + 3, { width: W * 0.2 - 4, align: 'right' })
      y += 14
      doc.rect(L, y, W, 14).fillAndStroke('#f5f5f5', '#ddd')
      doc.fillColor('#000').fontSize(8).font('Helvetica-Bold')
        .text('TOTAL:', L + W * 0.75, y + 3, { width: W * 0.25 - 4 })
        .text(`R$  ${fmtBRL(totais.valorIssqn)}`, L + W * 0.75, y + 3, { width: W * 0.25 - 8, align: 'right' })
      y += 14
    }

    drawBlock(`VALOR LÍQUIDO (R$) - Nota Fiscal${pctNfLabel}`, liquidoNf, true)

    if (totais.pctLocacao > 0) {
      drawBlock(`VALOR BRUTO (R$) - Fatura de Locação${pctLocLabel}`, totais.valorLocacao, true)
      drawBlock(`VALOR LÍQUIDO (R$) - Fatura de Locação${pctLocLabel}`, totais.valorLocacao, true)
    }
    ...
    if (medicao.responsavel_medicao) {
      doc.fontSize(8).font('Helvetica').fillColor('#444')
        .text(`Responsável pela medição: ${medicao.responsavel_medicao}`, L, y)
      y += 12
    }
    if (medicao.conferido_por) {
      doc.text(`Conferido por: ${medicao.conferido_por}`, L, y)
    }
    ...
    res.setHeader('Content-Type', 'application/pdf')
    res.setHeader('Content-Disposition', `inline; filename="medicao-${medicao.numero}-${medicao.obra_numero}.pdf"`)
    res.send(buf)
  } catch (e) { err(res, e.message, 500) }
})
