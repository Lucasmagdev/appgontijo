...
async function calcularTotaisMedicao(medicaoId) {
  ...
  const [diasRows] = await db.query(
    'SELECT DATE_FORMAT(data, \'%Y-%m-%d\') AS data, observacao FROM medicao_dias WHERE medicao_id = ?',
    [medicaoId]
  ).catch(() => [[]])
  const obsByDate = {}
  for (const d of diasRows) obsByDate[d.data] = d.observacao || null
...
function buildMedicaoPdfBuffer(detalhe) {
  return new Promise((resolve, reject) => {
    ...
    const cols = [
      ...
      { label: 'Total',          w: hasObservacao ? 0.085 : 0.160 },
      ...(hasObservacao ? [{ label: 'Observação', w: 0.150 }] : []),
    ]
    ...
    if (fatMinimoTable.length > 0 && detalhe.totais.valorFatMinimo > 0) {
      ...
      for (const item of fatMinimoTable) {
        ...
        doc.font('Helvetica').fontSize(6).fillColor(color).text(dateLabel, fX[0] + 2, y + 3, { width: fW[0] - 4, lineBreak: false })
        doc.font('Helvetica').fontSize(6).fillColor(color).text(item.producao > 0 ? `R$ ${fmtBRL(item.producao)}` : `R$ -`, fX[1] + 2, y + 3, { width: fW[1] - 4, align: 'right', lineBreak: false })
        doc.font('Helvetica').fontSize(6).fillColor('#111').text(item.saldo > 0 ? `R$ ${fmtBRL(item.saldo)}` : `R$ -`, fX[2] + 2, y + 3, { width: fW[2] - 4, align: 'right', lineBreak: false })
        doc.font('Helvetica').fontSize(6).fillColor('#555').text(String(item.observacao || ''), fX[3] + 2, y + 3, { width: fW[3] - 4, lineBreak: false })
        y += RH
      }
    ...
    // NF / Locação split rows
    function pdfSplitRow(label, valor, color) {
      ensureY(RH)
      doc.save(); doc.fillColor('#fafafa').rect(L, y, W - 130, RH).fill(); doc.restore()
      doc.save(); doc.strokeColor(BORDER).lineWidth(0.3).rect(L, y, W - 130, RH).stroke(); doc.restore()
      doc.save(); doc.fillColor('#fafafa').rect(L + W - 130, y, 130, RH).fill(); doc.restore()
      doc.save(); doc.strokeColor(BORDER).lineWidth(0.3).rect(L + W - 130, y, 130, RH).stroke(); doc.restore()
      vline(L + W - 130, y, y + RH)
      doc.font('Helvetica').fontSize(6.5).fillColor(color || '#333').text(label, L + 4, y + 3, { width: W - 138, lineBreak: false })
      doc.font('Helvetica').fontSize(6.5).fillColor(color || '#333').text(`R$ ${fmtBRL(valor)}`, L + W - 128, y + 3, { width: 122, align: 'right', lineBreak: false })
      y += RH
    }

    if (totais.pctNf > 0) {
      pdfSplitRow(`Fatura NF (${fmtBRL(totais.pctNf)}%)`, totais.valorNf)
    }
    if (totais.pctLocacao > 0) {
      pdfSplitRow(`Fatura de Locação (${fmtBRL(totais.pctLocacao)}%)`, totais.valorLocacao)
    }
    ...
    const brutoLiquidoRows = [['VALOR BRUTO (R$) - NF', valorBrutoNf], ['VALOR LÍQUIDO (R$) - NF', valorLiquidoNf]]
    if (totais.pctLocacao > 0) brutoLiquidoRows.push(['VALOR (R$) - LOCAÇÃO', totais.valorLocacao])
    ...
    ensureY(70)
    y += 18
    const sigLineW = W / 2 - 24
    const col2X = L + W / 2 + 14
    ...
    doc.font('Helvetica').fontSize(6.5).fillColor('#888')
       .text(`Emitido em ${new Date().toLocaleDateString('pt-BR')}`, L, y + 42, { width: W, align: 'right', lineBreak: false })
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT m.id, m.construction_id, m.numero, m.data_medicao, m.data_inicio, m.data_fim,
              m.responsavel_medicao, m.conferido_por, m.status, m.criado_em,
              c.construction_number AS obra_numero,
              COALESCE(cl.name,'') AS cliente
       FROM medicoes m
       LEFT JOIN constructions c ON c.id = m.construction_id
...
router.post('/medicoes', requireGontijoAuth, async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO medicoes (construction_id, numero, data_medicao, data_inicio, data_fim, responsavel_medicao, conferido_por)
       VALUES (?, ?, CURDATE(), ?, ?, ?, ?)`,
      [obraId, proximo, dataInicio, dataFim, textOrNull(responsavelMedicao), textOrNull(conferidoPor)]
    )
