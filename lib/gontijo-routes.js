...
function buildMedicaoPdfBuffer(detalhe) {
  return new Promise((resolve, reject) => {
    ...
    const infoRows = [
      ...
      [['', ''], ['N° DO CONTRATO:', medicao.obra_numero], ['N° DA MEDIÇÃO:', String(medicao.numero || '')], ['N° DA REVISÃO:', '']],
    ]
    ...
    // Conferido por footer
    ensureY(18)
    y += 4
    doc.font('Helvetica').fontSize(6.5).fillColor('#666')
       .text(`Conferido por: ${medicao.conferido_por || ''}`, L, y, { width: W / 2, lineBreak: false })
    doc.font('Helvetica').fontSize(6.5)
       .text(`Emitido em ${new Date().toLocaleDateString('pt-BR')}`, L + W / 2, y, { width: W / 2, align: 'right', lineBreak: false })
