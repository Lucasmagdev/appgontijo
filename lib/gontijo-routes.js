...
async function calcularTotaisMedicao(medicaoId) {
  ...
  // Load considera_fat_minimo per diary (for estacas)
  const diaryDateExpr = "COALESCE(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(d.data, '$.date')), ''), DATE_FORMAT(d.created_at, '%Y-%m-%d'))"
  const [diaryObs] = medicao.data_inicio && medicao.data_fim
    ? await db.query(
        `SELECT ${diaryDateExpr} AS data_diario, d.considera_fat_minimo
         FROM diaries d
         WHERE CAST(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(d.data, '$.construction_id')), '') AS UNSIGNED) = ?
  ...
  for (const d of diaryObs) {
    diaryByDate[d.data_diario] = { consideraFatMinimo: Boolean(d.considera_fat_minimo) }
  }
  ...
  for (const data of allDates) {
    ...
    fatMinimoTable.push({
      ...
      saldo,
      consideraFatMinimo,
      observacao: obsByDate[data] ?? null,
