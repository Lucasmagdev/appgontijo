...
async function calcularTotaisMedicao(medicaoId) {
  ...
  return { medicao, estacas, extras, fatMinimoTable, totais: { valorEstacas, valorFatMinimo: totalFatMinimo, valorExtras, valorTotal } }
}
...
function buildMedicaoPdfBuffer(detalhe) {
  return new Promise((resolve, reject) => {
    ...
    // Valor Bruto / Líquido
    for (const label of ['VALOR BRUTO (R$) - NF', 'VALOR LÍQUIDO (R$) - NF']) {
      ensureY(22)
      ...
      doc.font('Helvetica-Bold').fontSize(8).text(`R$ ${fmtBRL(totais.valorTotal)}`, L + W - 128, y + 6, { width: 122, align: 'right', lineBreak: false })
      y += 20 + 4
    }
...
router.put('/medicoes/:id', requireGontijoAuth, async (req, res) => {
  try {
    ...
    const { responsavelMedicao, conferidoPor, dataMedicao, dataInicio, dataFim, itensExtras } = req.body
    await db.query(
      ...
      [textOrNull(responsavelMedicao), textOrNull(conferidoPor),
       textOrNull(dataMedicao), textOrNull(dataInicio), textOrNull(dataFim), id]
    )
