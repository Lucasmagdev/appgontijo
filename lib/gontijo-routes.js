...
router.post('/usuarios', requireGontijoAdmin, async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      ...
      [nome, textOrNull(apelido), login, textOrNull(telefone), textOrNull(cargo), senha_hash, perfil === 'admin' ? 'S' : 'S', mapBooleanToLegacyFlag(podeGerarLinkAssinatura)]
    )
...
router.put('/usuarios/:id', requireGontijoAdmin, async (req, res) => {
  try {
    await ensureUserSignaturePermissionColumn()
    const { nome, apelido, login, telefone, cargo, status, podeGerarLinkAssinatura } = req.body
    await db.query(
      'UPDATE users SET name = ?, alias = ?, document = ?, phone = ?, cargo = ?, active = ?, pode_gerar_link_assinatura = ? WHERE id = ?',
      [nome, textOrNull(apelido), login, textOrNull(telefone), textOrNull(cargo), mapStatusToLegacyActive(status), mapBooleanToLegacyFlag(podeGerarLinkAssinatura), req.params.id]
    )
    ok(res, {})
  } catch (e) {
    if (e.code === 'ER_DUP_ENTRY') return err(res, 'Login ja cadastrado')
  ...
  try {
    ...
    const [[existingDraft]] = await conn.query(
      `SELECT d.id
       ...
       ORDER BY d.id DESC
       LIMIT 1`,
      [operadorId, equipamentoId, Number(construction.id), today]
    )
    ...
    const existingDiarySameDay = await findDiaryByEquipmentAndDate(conn, {
      equipmentId: equipamentoId,
      date: today,
    })

    if (existingDiarySameDay?.id) {
      return err(res, 'Diário duplicado: já existe um diário para este equipamento na data de hoje', 409)
    }
