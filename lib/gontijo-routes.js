...
function mapLegacyActiveToStatus(value) {
  return String(value || '').toUpperCase() === 'N' ? 'inativo' : 'ativo'
}
...
function mergeDiaryJson(row, options = {}) {
  ...
  if (options.staffRows) {
    const existingFreeText = normalizeDiaryStaffInput(parsed.staff).filter((item) => item.usuario_id === null)
    const mergedRows = [...options.staffRows]

    existingFreeText.forEach((item) => {
      if (!mergedRows.some((rowItem) => rowItem.usuario_id === null && rowItem.nome_membro === item.nome_membro)) {
...
async function loadComputerImeiMap() {
  const imeiColumn = await findFirstExistingColumn('computers', ['imei', 'serial', 'serial_number', 'identifier'])
  if (!imeiColumn) return new Map()

  const [rows] = await db.query(`SELECT id, ${imeiColumn} AS imei FROM computers`)
  return new Map(rows.map((row) => [Number(row.id), textOrNull(row.imei) || '']))
}
...
async function buildLegacyEquipmentRows({ onlyBound = false } = {}) {
  const imeiMap = await loadComputerImeiMap()
  ...
  const items = rows.map((row) => {
    const binding =
      bindings.byEquipmentId.get(Number(row.id)) ||
    ...
    return {
      ...
      computador_geo: intOrNull(row.computer_id) ? String(row.computer_id) : '',
      ...
      imei: imeiMap.get(Number(row.computer_id)) || '',
...
router.get('/equipamentos', async (req, res) => {
  try {
    const rows = await buildLegacyEquipmentRows({ onlyBound: req.query.parametrizados === '1' })
    ok(res, { data: rows })
  } catch (e) {
    err(res, e.message, 500)
  }
...
router.post('/equipamentos', async (req, res) => {
  ...
  try {
    ...
    const [result] = await conn.query(
      ...
      [nome, intOrNull(computador_geo), intOrNull(modalidade_id), mapStatusToLegacyActive('ativo')]
    )
...
router.put('/equipamentos/:id', async (req, res) => {
  ...
  try {
    ...
    await conn.query(
      ...
      [nome, intOrNull(computador_geo), intOrNull(modalidade_id), mapStatusToLegacyActive(status), req.params.id]
    )
...
router.get('/diarios', async (req, res) => {
  try {
    ...
    if (data_inicio) {
      where += " AND COALESCE(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(d.data, '$.date')), ''), DATE_FORMAT(d.created_at, '%Y-%m-%d')) >= ?"
    ...
    if (obra) {
      where += " AND COALESCE(c.construction_number, JSON_UNQUOTE(JSON_EXTRACT(d.data, '$.construction_number'))) LIKE ?"
      params.push(`%${obra}%`)
    }

    if (equipamento_id) {
      where += " AND CAST(COALESCE(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(d.data, '$.equipment_id')), ''), '0') AS UNSIGNED) = ?"
    ...
    if (status) {
      where += " AND COALESCE(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(d.data, '$.status')), ''), 'pendente') = ?"
      params.push(status)
    }
...
router.get('/diarios/:id', async (req, res) => {
  try {
    ...
    const staffMap = await listDiaryStaffMap([row.id])
    ok(res, { data: hydrateDiaryRow(row, { includeJson: true, staffRows: staffMap.get(Number(row.id)) || [] }) })
  } catch (e) {
    err(res, e.message, 500)
  }
})
