...
const ADMIN_CPFS = new Set(['09653344650'])

function requireGontijoAdmin(req, res, next) {
  const cpf = req.session && req.session.adminCpf
  if (!cpf || !ADMIN_CPFS.has(cpf)) {
    ...
    const [result] = await db.query(
      `UPDATE diaries
       SET conferencia_status = 'aprovado', conferencia_em = NOW(), conferencia_por = ?, conferencia_obs = ?,
           producao_real = ?, valor_faturado = ?
       WHERE id = ?`,
      [adminId, obs, valores?.producaoReal ?? null, valores?.valorFaturado ?? null, id]
    )
    if (result.affectedRows === 0) return err(res, 'Diário não encontrado', 404)
    ok(res, {})
  } catch (e) {
    ...
    const [result] = await db.query(
      `UPDATE diaries
       SET conferencia_status = 'rejeitado', conferencia_em = NOW(), conferencia_por = ?, conferencia_obs = ?,
           producao_real = NULL, valor_faturado = NULL
       WHERE id = ?`,
      [adminId, obs, id]
    )
    if (result.affectedRows === 0) return err(res, 'Diário não encontrado', 404)
